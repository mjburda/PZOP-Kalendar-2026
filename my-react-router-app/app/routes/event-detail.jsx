...
export async function loader({ params }) {
  // ziskame id ukolu z adresy url
  const { id } = params;
  try {
    const res = await sql(`SELECT * FROM pzop_event WHERE id = ${id} LIMIT 1`);
    const predmety = await sql("SELECT * FROM pzop_subject");

    // kdys uz to neexistuje tk to hodi na domovskou starnku
    if (!res[0]) return redirect("/");
    return { udalost: res[0], predmety: predmety || [] };
  } catch (e) {
    console.error(e);
    return redirect("/");
  }
...
export async function action({ request, params }) {
  ...
  // tohle rzi jestli se dalo ulozit nbo vymazat
  ...
  if (intent === "save") {
    ...
    await sql(`
      UPDATE pzop_event 
      SET title = '${title}', event_date = '${event_date}', subject_shortcut = '${subject_shortcut}', description = '${description}', is_test = ${is_test}
      WHERE id = ${id}
    `);
  }

  // odstaneni upe celyho ukolu
  if (intent === "delete") {
...
export default function EventDetail() {
  const { udalost, predmety } = useLoaderData();
      ...
      <section className="w-full max-w-3xl bg-[#f4f5f9] min-h-screen md:min-h-fit md:rounded-[2.5rem] md:shadow-2xl border-gray-200/50 md:border p-6 md:p-10 flex flex-col justify-center">
        <article className="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 md:p-10 flex flex-col">
          <header className="flex justify-between items-center mb-8 border-b border-gray-100 pb-6">
            <h2 className="font-bold text-gray-800 text-xl">Upravit událost</h2>
            <Link
              to="/"
          ...
          >
            {/* ciste zapsany labely at tu neni milion vnorenych html div tagu, na PC to hodi 2 vedle sebe */}
            ...
            <label className="flex flex-col gap-2 border-b border-gray-200 pb-2 focus-within:border-blue-500 cursor-text group">
              ...
              <input
                type="date"
                name="event_date"
                defaultValue={udalost.event_date}
                required
                className="w-full bg-transparent outline-none text-sm font-medium text-gray-800"
              />
            </label>
            ...
            <label className="flex flex-col gap-2 border-b border-gray-200 pb-2 focus-within:border-blue-500 cursor-pointer group md:col-span-2">
              ...
              <select
                ...
                defaultValue={udalost.subject_shortcut}
                required
            ...
            <label className="flex flex-col gap-2 border-b border-gray-200 pb-2 focus-within:border-blue-500 cursor-text group md:col-span-2">
              ...
              <textarea
                name="description"
                rows="4"
                defaultValue={udalost.description}
            ...
            <label className="flex items-center gap-3 p-4 bg-gray-50 rounded-2xl select-none cursor-pointer md:col-span-2 border border-transparent hover:border-gray-200 transition-all">
              <input
                type="checkbox"
                name="is_test"
                defaultChecked={udalost.is_test === 1}
            ...
            <footer className="pt-6 flex flex-col sm:flex-row justify-end gap-4 md:col-span-2">
              <button
                type="submit"
                name="intent"
                value="delete"
                onClick={(e) => {
                  if (!confirm("Smazat naporad?")) e.preventDefault();
                }}
                className="bg-white text-red-500 hover:bg-red-50 hover:text-red-600 px-6 py-4 rounded-full font-bold text-sm flex items-center justify-center gap-2 border border-red-100 transition-colors sm:mr-auto"
              >
              ...
              <Link
                to="/"
                className="text-sm font-bold text-gray-500 px-6 py-4 rounded-full border border-gray-200 bg-white hover:bg-gray-50 transition-colors text-center"
              >
                Zrušit
              </Link>
              ...
              >
                <span className="material-icons text-lg">save</span> Uložit
              </button>
            </footer>
          </Form>
        </article>
